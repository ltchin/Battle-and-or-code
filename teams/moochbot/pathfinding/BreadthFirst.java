...
//			}
	...
	public static Direction getNextDirection(ArrayList<MapLocation> givenPath, int bigBoxSize){
		//just check the bottom member of the path, to see if it needs truncating
		if(givenPath.size() > 1 && VectorFunctions.mldivide(rc.getLocation(),bigBoxSize).equals(givenPath.get(0))){
			givenPath.remove(0);
		}
		return rc.getLocation().directionTo(VectorFunctions.bigBoxCenter(givenPath.get(0),bigBoxSize));
	}
